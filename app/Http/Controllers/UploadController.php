...
class UploadController extends Controller
{
    ...
    public function uploadBanner(Request $request)
    {
        ...
        if (count($banner) >= 5) {
            ...
            if ($request->file()) {
                ...
                $request->file('file')->move('uploads', $file_name);
                $fileUpload->img_name = 'banner' . '_' . time() . '_' . $request->file->getClientOriginalName() . '.jpg';
                $fileUpload->url = 'uploads/' . $file_name;
                $fileUpload->save();
                return response()->json(['success' => 'File uploaded successfully.', 'path' => $fileUpload->url]);
            }
    ...
    public function uploadThumb(Request $request)
    {
        ...
        $fileUpload = new Upload;
        if ($request->file()) {
            $file_name = 'thumb' . '_' . time() . '_' . $request->file->getClientOriginalName() . '.jpg';
            $request->file('file')->move('uploads', $file_name);
            $fileUpload->img_name = 'thumb' . '_' . time() . '_' . $request->file->getClientOriginalName() . '.jpg';
            $fileUpload->url = 'uploads/' . $file_name;
            $fileUpload->save();
            return response()->json(['success' => 'File uploaded successfully.', 'path' => $fileUpload->url]);
        }
